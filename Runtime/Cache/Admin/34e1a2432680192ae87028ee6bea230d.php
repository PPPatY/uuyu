...
<!--下拉样式结束-->
	...
	<div class="cf top_nav_list">
		<div class="fl button_list">
			<?php if(empty($model["extend"])): ?><div class="tools">
					<a class=" " href="<?php echo U('send',['promote_type'=>1]);?>"><span class="button_icon button_icon1"></span>给一级推广员发放</a>
					<a class=" " href="<?php echo U('send',['promote_type'=>2]);?>"><span class="button_icon button_icon1"></span>给二级推广员发放</a>
				</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="jssearch fr cf search_list">
			...
			<div class="input-list">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('send_lists','model='.$model['name'],false);?>">搜索</a>
			</div>
		</div>
	</div>
	...
	<div class="data_list">
		<div class="">
			<table>
				...
				<thead>
					<tr>
						<?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field["title"] != '来源'): ?><th><?php echo ($field["title"]); ?></th><?php endif; ?>
							<?php if($key == 1): ?><th>发放类型</th><?php endif; endforeach; endif; else: echo "" ;endif; ?>
					</tr>
				</thead>
				...
				<?php if(!empty($list_data)): if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
							<?php if(is_array($list_grids)): $i = 0; $__LIST__ = $list_grids;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$grid): $mod = ($i % 2 );++$i; if($grid["title"] != '来源'): ?><td><?php echo get_list_field($data,$grid);?></td><?php endif; ?>
								<?php if($key == 1): ?><td>平台币</td><?php endif; endforeach; endif; else: echo "" ;endif; ?>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					<tr class="data_summary">
						<td colspan="9"><span style="margin-right: 10px;">汇总：</span><span>当页发放：<?php echo null_to_0(array_sum(array_column($list_data,'num')));?>个 ；今日发放：<?php echo ((isset($sum["to_day"]) && ($sum["to_day"] !== ""))?($sum["to_day"]):0); ?>个 ； 昨日发放：<?php echo ((isset($sum["yst_day"]) && ($sum["yst_day"] !== ""))?($sum["yst_day"]):0); ?>个 ； 累计发放：<?php echo ((isset($sum["all_num"]) && ($sum["all_num"] !== ""))?($sum["all_num"]):0); ?>个</span></td>
	...
	<script type="text/javascript">
		highlight_subnav('<?php echo U('PromoteCoin/send_lists');?>');
