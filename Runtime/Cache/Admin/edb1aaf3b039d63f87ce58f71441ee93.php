...
<!--下拉样式结束-->
	...
	<div class="main-place">
		...
		<ul class="main-placeul">
			...
			<li>
				<a href="<?php echo U('PromoteCoin/send_lists');?>">平台币收回</a>
			</li>
			<li>
				<a href="#">收回平台币</a>
			</li>
	...
	<!-- 标签页导航 -->
	<div class="tab-wrap">
		<div class="formtitle" style="margin-bottom: 0;"><span>收回平台币</span><p class="description_text" style="line-height: 20px;">说明：此功能是收回<?php if(I('detype') == 2): ?>推广员<?php else: ?>玩家<?php endif; ?>平台币的功能</p></div>
		<div class="tab-content tab_content">
			<!-- 表单 -->
			<form id="form" action="<?php echo U('deduct',array('detype'=>I('detype'),'model'=>$model['id']));?>" method="post" class="form-horizontal">
				<!-- 基础文档模型 -->
				<div id="tab1" class="tab-pane in tab1 tab_table">
					<table border="0" cellspacing="0" cellpadding="0">
						<tbody>
							<?php if(I('detype') == 2): ?><tr>
									<td class="l">推广员账号：</td>
									<td class="r">
										<select id="promote_id" name="promote_id">
											<?php $_result=promote_lists();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option coin-yue='<?php echo ($vo["balance_coin"]); ?>' value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["account"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
									</td>
								</tr>
								<?php else: ?>
	...
	<script type="text/javascript">
		...
		$('#account').bind('input propertychange', function() {
			$.post("<?php echo U('Member/user_balance');?>", { id: $('#account').val(), type: 1 }, function(result) {
				if(result.status == 0) {
		...
		function getDiscount() {
			$.ajax({
				url: "<?php echo U('Ajax/getGameDiscount');?>",
				type: 'post',
				dataType: 'json',
		...
		<?php if(I('detype') == 2): ?>$(".coin_yue").text($('#promote_id option:selected').attr('coin-yue')+'平台币');
<?php else: ?>
    $(".coin_yue").text($('#account').attr('coin-yue')+'平台币');<?php endif; ?>
	</script>
