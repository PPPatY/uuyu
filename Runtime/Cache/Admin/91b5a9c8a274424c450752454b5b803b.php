...
.tab_nav .tab-nav {}
	...
	<div class="main-place">
        ...
        <ul class="main-placeul">
            <li><a href="<?php echo U('Config/group');?>">站点</a></li>
            <li><a href="<?php echo U('Site/channel');?>">推广员后台</a></li>
            <li><a href="#">基本配置</a></li>
        </ul>
    ...
    <ul>
		<?php if(is_array(C("CONFIG_GROUP_LIST"))): $i = 0; $__LIST__ = C("CONFIG_GROUP_LIST");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$group): $mod = ($i % 2 );++$i;?><li <?php if(($id) == $key): ?>class="current"<?php endif; ?>><a href="<?php echo U('channel',array(cate_id=>I('cate_id'),group_id=>$key));?>"><?php echo ($group); ?>配置</a></li><?php endforeach; endif; else: echo "" ;endif; ?>
		</ul></div>
		<div class="tab-content tab_content">
	<form action="<?php echo U('save');?>" method="post" class="form-horizontal">
    <ul class="form_info form_info<?php echo I('group_id',1)+10;?>">
    <?php if(empty($list)): ?>暂无配置
	<?php else: ?>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><li>
			...
			<?php case "2": ?><textarea name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea><?php break;?>
			<?php case "3": ?><textarea name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config["value"]); ?></textarea><?php break;?>
            ...
            <i class="form_hint"><?php echo ($config["remark"]); ?></i>
        </li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
        </ul>
		<div class="form-item">
			<label class="item-label"></label>
			<div class="controls">
				<?php if(empty($list)): ?><button type="submit" disabled class=" submit_btn disabled" target-form="form-horizontal">确 定</button><?php else: ?><button type="submit" class=" submit_btn ajax-post" target-form="form-horizontal">确 定</button><?php endif; ?>
...
<script type="text/javascript">
	//导航高亮
	highlight_subnav('<?php echo U('Site/channel');?>');
</script>
