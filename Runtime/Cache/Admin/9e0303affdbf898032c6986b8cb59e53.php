...
<!--下拉样式结束-->
    ...
    <div class="tab-wrap">
        <div class="tab_nav jstabnav">
            <ul>
                <li data-tab="tab1" class="current firsttab"><a href="javascript:void(0);" >上传永久图片</a></li>
            ...
            <form action="<?php echo U('saveTool');?>" class="form-horizontal qq_login form_info_ml">
                <div id="tab1" class="tab-pane in tab1">
                    ...
                    <div class="bg-pic li-com" style="margin-bottom: 25px;">
                        <span class="key-text" style="margin-top: -3px;">类型：</span>
                        <label for="for-tuwen" class="for-tuwen js-change-keyw">
                            <input id="for-tuwen" type="radio" value="image" name="chose" checked="checked">图片
                        </label>
                        <label for="for-text" class="for-text js-change-keyw">
                            <input id="for-text" type="radio" value="thumb" name="chose">缩略图
                        </label>
                    </div>
                    <div class="bg-pic li-com">
                        <span class="key-text">备注：</span>
                ...
                <div id="tab2" class="tab-pane in tab2">
                    ...
                    <div class="keyword li-com">
                        ...
                        <input type="text" class="Meterial-item-p" value="" placeholder="1,2,3,4,5" style="padding: 3px 6px; border:1px solid #999;">
                        <i style="font-style: normal;color: #666; margin-left: 30px;">图文消息列表ID,以 ， 隔开，最多不能超过8个</i>

                    </div>
                    <div class="bg-pic li-com" style="margin-top: 20px;">
                        <span class="key-text">备注：</span>
                        <textarea id="editor-2" style="border:1px solid gray;min-height:240px;max-width: 600px;"></textarea>
                    </div>
                    <div class="form-item">
                        <label class="item-label"></label>
                        <div class="controls">
                            <div class="submit-btn js-submit-item-p" target-form="form-horizontal" style="width: 36px; cursor: pointer;">确 定</div>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        $('.js-change-keyw').click(function () {
            var Sshow = $(this);
            data.type = Sshow.hasClass('for-tuwen') ? "image" : "thumb" ;
        });

        // 点击提交 获取列表2数据
        $('.js-submit-item-p').click(function () {
            var data2 = {};
            data2.ids = $('.Meterial-item-p').val();
            data2.mark = $('#editor-2').val();
            sendDatas(data2);
        });
        // 点击提交 获取列表1数据
        $('.js-submit-menuSettings').click(function () {
            data.url = $('.upload-img-box .upload-pre-item img')[0].src.replace(/^http:\/\/[^/]+/, "") || null;
            data.mark = data.mark || $('#editor-w').val();
        ...
        function sendDatas(e){
            ...
            var data_v = e;
            var sendData = $.ajax({
                url: "<?php echo U('Wxoperate/upMeterial');?>",
                method: 'post',
                data: data_v,
                dataType: 'json'
            });
