...
<script type="text/javascript" src="/Public/Admin/js/select2.min.js"></script>
    ...
    <div class="cf top_nav_list">
        ...
        <?php if(empty($model["extend"])): ?><div class="tools">
                <a class=" ajax-post " target-form="ids" url="<?php echo U("Apply/app_audit");?>"><span class="button_icon button_icon9"></span>审 核</a>
                <a class=" ajax-post  pack" id="org" target-form="ids" url="<?php echo U("Apply/app_package");?>" ><span class="button_icon button_icon23"></span>打 包</a>
                <a class=" ajax-post confirm " target-form="ids" url="<?php echo U('app_del?model='.$model['id']);?>"><span class="button_icon button_icon2"></span>删 除</a>
            </div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
        <div class="jssearch fr cf search_list">
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Apply/app_lists','model='.$model['name'],false);?>">搜索</a>
            </div>
        </div>
    </div>
    <!-- 数据列表 -->
    <div class="data_list">
        <div class="">
            <table>
                ...
                <!-- 列表 -->
                <tbody>
                <?php if(empty($list_data)): ?><td colspan="10" class="text-center"> aOh! 暂时还没有内容! </td>
                <?php else: ?>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td>
                                <?php if($data['app_version'] == 1): ?>安卓<?php endif; ?>
                                <?php if($data['app_version'] == 0): ?>苹果<?php endif; ?>
                            </td>
                            <td><?php echo set_show_time($data['apply_time']);?></td>
                            <td><?php if($data["status"] == 0): ?><span style="color: orange"><?php echo get_info_status($data['status'],5);?></span><?php else: echo get_info_status($data['status'],5); endif; ?></td>
                            <td><?php if($data["dow_url"] == ''): ?><span style="color: orange">未打包</span><?php else: ?>已打包<?php endif; ?></td>
                            <td><?php echo get_admin_nickname('',$data['dispose_id']);?></td>
                            <td><?php echo set_show_time($data['dispose_time'],'','other');?></td>
                            <td>
                               <a href="<?php echo U('Apply/app_package?ids='.$data['id']);?>" class="ajax-get  pack">打包</a>
                               <a href="" class="ajax-get  popbtn" data-url="<?php echo ($data["dow_url"]); ?>">查看</a>
                            </td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Apply/app_lists');?>');
$(function(){
    ...
    $('.popbtn').on('click',function() {
        ...
        if(url.indexOf("./") == 0){
            var urll="http://"+"<?php echo ($_SERVER['HTTP_HOST']); ?>"+url;
        }else{
            var urll = url;
        }
        if(url){
