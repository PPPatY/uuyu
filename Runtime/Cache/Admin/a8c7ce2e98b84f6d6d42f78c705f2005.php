	...
	<div class="cf top_nav_list">
		<div class="fl button_list">
			<?php if(empty($model["extend"])): ?><div class="tools data_list">
					...
					<a class=" ajax-post " target-form="ids" url="<?php echo U("Promote/set_status",array("status"=>1,"msg_type"=>5,"field"=>"status"));?>"><span class="button_icon button_icon9"></span>审 核</a>

					<label class="checked">
						<?php if($PROMOTE_AUTO_AUDIT == 0): ?><input class="tabok AUTO_AUDIT" data-val="<?php echo ($PROMOTE_AUTO_AUDIT); ?>" type="checkbox" value="" name="">
				        <?php else: ?>
				            <input class="tabok AUTO_AUDIT" data-val="<?php echo ($PROMOTE_AUTO_AUDIT); ?>" type="checkbox" checked value="" name=""><?php endif; ?>
		...
		<div class="jssearch fr cf search_list">
			<div class="input-list input-list-promote search_label_rehab">
				...
				<select id="promote_id" name="account" class="select_gallery">
					...
					<?php $_result=get_promote_list(1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option promote-id="<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["account"]); ?>"><?php echo ($vo["account"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			<div class="input-list input-list-parent search_label_rehab">
				...
				<select id="parent_id" name="parent_id" class="select_gallery">
					<option value="">请选择</option>
					<?php $_result=get_all_toppromote();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option parent-id="<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["account"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			...
			<div class="input-list">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/lists','model='.$model['name'],false);?>">搜索</a>
			</div>
             <div class="input-list search_item"> 
                <a class="export-btn" href="<?php echo U('Export/expUser',array_merge(array('id'=>23,),I('get.')));?>">导出</a>
            </div>
		</div>
	</div>
	<!-- 数据列表 -->
			...
			<table>
				...
				<tbody>
					...
					<?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
							...
							<td style="text-align:center"><?php echo ($data["account"]); ?></td>
							<td style="text-align:center"><?php echo ($data["balance_coin"]); ?></td>
							<td style="text-align:center"><?php echo promote_user_pay($data['id']);?>元</td>
							<td style="text-align:center"><?php echo ($data["mobile_phone"]); ?></td>
							<td style="text-align:center">
								<?php if($data["parent_id"] == 0): echo get_qu_promote($data['parent_id']);?>
									<?php else: ?><span><?php echo get_qu_promote($data['parent_id']);?></span><?php endif; ?>
							</td>
							<td style="text-align:center"><?php echo get_top_promote($data['id'],$data['parent_id']);?></td>
							<td style="text-align:center">
								<?php if(($data['busier_id']) == "0"): ?>--
									<?php else: echo get_busier_account($data['busier_id']); endif; ?>
							</td>
							<td style="text-align:center">
								<?php if($data['status'] == 0): ?><span class="preview_status"><?php echo get_info_status($data['status'],3);?></span>
									<?php elseif($data['status'] == 2 ): ?>
									<span class="disabled_status"><?php echo get_info_status($data['status'],3);?></span>
									<?php else: ?> <?php echo get_info_status($data['status'],3); endif; ?>
							</td>
							<td style="text-align:center"><?php echo set_show_time($data['create_time']);?></td>
							<td style="text-align:center">
								<a href="<?php echo U('edit',array('id'=>$data['id']));?>" class="">编辑</a>
								<?php if($data["status"] != 2): ?><a href="<?php echo U('Promote/set_status',array('ids'=>$data['id'],'status'=>2,'msg_type'=>11,'field'=>'status'));?>" class="ajax-get">锁定</a>
									<?php else: ?>
									<a href="<?php echo U('Promote/set_status',array('ids'=>$data['id'],'status'=>1,'msg_type'=>12,'field'=>'status'));?>" class="ajax-get">解锁</a><?php endif; ?>
							</td>
			...
			$(".AUTO_AUDIT").click(function() {
				...
				layer.msg(tip, {
					...
					yes: function(index) {
						layer.closeAll();
						$.ajax({
							url: "<?php echo U('change_auto_audit');?>",
							type: 'post',
							data: { value: $AUDIT },
							async: false,
							success: function(data) {
