...
<!--下拉样式结束-->
    ...
    <div class="tab-wrap">
        <div class="tab_nav jstabnav">
            <ul>
                <li data-tab="tab1" class="current firsttab"><a href="javascript:void(0);" >永久图文素材</a></li>
            </ul>
        </div>
        ...
        <div class="tab-content">
            <form action="<?php echo U('saveTool');?>" class="form-horizontal qq_login form_info_ml">
                <div id="tab1" class="tab-pane in tab1">
                    <ul class="form_info ">
                        <li>
                            <label style="width: 50px;">列表：</label>
                            <a href="<?php echo U('Wxoperate/addNewsMeterial');?>">
                                <input type="button" value="添加" class=""style="padding:5px 20px; margin-left:30px;" >
                            </a>
                        </li>
                        <table border="1" class="table-list" style="margin-left: 50px">
                            ...
                            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$meterialData): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($meterialData["id"]); ?></td>
                                    <td><?php echo ($meterialData["title"]); ?></td>
                                    <td title="<?php echo ($meterialData["thumb_media_id"]); ?>"><?php echo ($meterialData["thumb_media_id"]); ?></td>
                                    <td><?php echo ($meterialData["author"]); ?></td>
                                    <td title="<?php echo ($meterialData["digest"]); ?>"><?php echo ($meterialData["digest"]); ?></td>
                                    <td>
                                        <?php if($meterialData["show_cover_pic"] == '0'): ?>否
                                        <?php else: ?>
                                            是<?php endif; ?>
                                    </td>
                                    <td title="<?php echo ($meterialData["content_source_url"]); ?>"><?php echo ($meterialData["content_source_url"]); ?></td>
                                    <td>
                                        <a href="javascript:;" class="js-delete" id="<?php echo ($meterialData["id"]); ?>" style="margin-right: 15px;">删除</a>
                                        <a href="?s=/Wxoperate/editnewsMeterial/id/<?php echo ($meterialData["id"]); ?>">编辑</a>
                                    </td>
    ...
    <script>
        ...
        function sendDatas(e,t){
            ...
            var data_v = e;
            var sendData = $.ajax({
                url: "<?php echo U('Wxoperate/delnewsMeterial');?>",
                method: 'post',
                data: data_v,
                dataType: 'json'
            })
