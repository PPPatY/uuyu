...
<style>
	...
	<div class="cf top_nav_list">
        ...
        <!--显示状态搜索-->
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('GameType/lists','model='.$model['name'],false);?>">搜索</a>
            </div>
		</div>
	</div>
    ...
    <div class="data_list">
        <div class="">
            <table>
                ...
                <!-- 列表 -->
                <tbody>
                    <?php if(empty($list_data)): ?><td colspan="6">暂无数据</td>
                    <?php else: ?>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php if($data['status_show'] == 0): ?><span style="color: red"><?php echo get_info_status($data['status_show']);?></span><?php else: echo get_info_status($data['status_show']); endif; ?></td>
                            <td><?php echo ($data["op_nickname"]); ?></td>
                            <td><?php echo set_show_time($data['create_time']);?></td>
                            <td>
                            <a href="<?php echo U('edit',array('model'=>$model['id'],'id'=>$data['id']));?>">编辑</a>
                            <?php if($data["status_show"] == 0): ?><a class="ajax-get" href="<?php echo U("GameType/set_status",array('ids'=>$data['id'],"status"=>1,"msg_type"=>2,"field"=>"status_show"));?>">开启</a>
                            <?php else: ?>
                            <a class="ajax-get" href="<?php echo U("GameType/set_status",array('ids'=>$data['id'],"status"=>0,"msg_type"=>3,"field"=>"status_show"));?>">关闭</a><?php endif; ?>
                            </td>
