...
<script type="text/javascript" src="/Public/Admin/js/select2.min.js"></script>
    ...
    <div class="cf top_nav_list navtab_list"> 
        <a <?php if(ACTION_NAME == 'lists'): ?>class="tabchose"<?php endif; ?> <?php if(I('group') != 2): ?>href="<?php echo U('Mend/lists');?>" <?php else: ?> href="<?php echo U('Provide/ptbsenduser');?>"<?php endif; ?> >推广补链</a>
        <a <?php if(ACTION_NAME == 'record'): ?>class="tabchose"<?php endif; ?> href="<?php echo U('Mend/record');?>">补链记录</a>
	...
	<div class="cf top_nav_list">
		...
		<div class="jssearch fr cf search_list">
            <div class="input-list"><label>玩家账号：</label>
                <input type="text" name="account" value="<?php echo I('account');?>" placeholder="请输入账号">
            </div>
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Mend/lists','model='.$model['name'],false);?>">搜索</a>
            </div>
        </div>
	</div>
                ...
                <tbody>
                    ...
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            <td style="text-align: center;">
                            	<label class="checked">
							        <input class="ids" type="checkbox" value="<?php echo ($data['uid']); ?>" name="ids[]">	
							        <i class="check_icon"></i>
						        </label>
                            </td>
                            <td style="text-align: center;"><?php echo ($data["account"]); ?></td>
                            <td style="text-align: center;"><?php echo (date('Y-m-d H:i:s',$data["register_time"])); ?></td>
                            <td style="text-align: center;"><?php echo (date('Y-m-d H:i:s',$data["login_time"])); ?></td>
                            <td style="text-align: center;"><?php echo ($data["register_ip"]); ?></td>
                            <td style="text-align: center;"><?php echo (date('Y-m-d H:i:s',$data["login_time"])); ?></td>
                            <td style="text-align: center;">
                            <?php if(get_parent_promoteto($data['promote_id']) != '' ): echo ($data['promote_account']); echo get_parent_promoteto($data['promote_id']);?>
                            <?php else: ?>
                                <?php echo get_promote_name($data['promote_id']); endif; ?></td>
                            <td><a href="<?php echo U('Mend/edit?id='.$data['id']);?>">补链</a></td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Mend/lists');?>');
$(function(){
	//搜索功能
