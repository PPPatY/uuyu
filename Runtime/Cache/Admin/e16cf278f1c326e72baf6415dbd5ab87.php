...
<style>
	...
	.submit_btn {margin-right: 30px;}
	.back_btn{margin-right: 30px;}
...
<div class="tab-wrap">
	...
	<form id="form" action="<?php echo U('update');?>" method="post" class="form-horizontal form_info_ml">
            ...
            <?php if(is_array($fields[$key])): $i = 0; $__LIST__ = $fields[$key];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i; if($field['name']=='comment'||$field['name']=='link_id'||$field['name']=='bookmark'||$field['name']=='template'){ unset($field); } ?>
                <?php if($field['is_show'] == 1 || $field['is_show'] == 3): ?><li>
                    <label><?php switch($field["name"]): case "belong_game": case "title": case "content": case "cover_id": ?><i class="must_input">*</i><?php break; endswitch; echo ($field['title']); ?></label>
                        <?php switch($field["type"]): case "num": ?><div class="form_main"><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>">
                                </div><?php break;?>
                            <?php case "string": ?><div class="form_main"><input type="text" class="text input-large" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>">
                                </div><?php break;?>
                            <?php case "textarea": ?><div class="form_main form_textarea_edit"><span class="form_textarea"><textarea name="<?php echo ($field["name"]); ?>"><?php echo ($field["value"]); ?></textarea></span>
                                </span></div><?php break;?>
                            <?php case "date": ?><div class="form_main"><input type="text" name="<?php echo ($field["name"]); ?>" class="text date" value="" placeholder="请选择日期" />
                                </div><?php break;?>
                            <?php case "datetime": ?><div class="form_main"><input type="text" name="<?php echo ($field["name"]); ?>" class="text time" value="" placeholder="请选择时间" />
                                </div><?php break;?>
                            <?php case "bool": ?><div class="form_main"><span class="form_select"><select name="<?php echo ($field["name"]); ?>" class="select_gallery select2-search--hide">
                            ...
                            <?php case "radio": ?><div class="form_main">
                                ...
                                <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="table_radio">
                                	<div class="radio radio-primary">
										<input type="radio" id="radio<?php echo ($key); ?>" value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>" <?php if(($field["value"]) == $key): ?>checked<?php endif; ?>> 
                            ...
                            <?php case "checkbox": ?><div class="form_main">
                                ...
                                <?php $_result=parse_field_attr($field['extra']);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="checked">
										<input type="checkbox" value="<?php echo ($key); ?>" name="<?php echo ($field["name"]); ?>[]" <?php if(($field["value"]) == $key): ?>checked<?php endif; ?>> 
                            ...
                            <?php default: ?>
                            <div class="form_main"><input type="text" class="text input-large" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>">
                            </div><?php endswitch;?>
		...
		<div class="form-item cf">
			...
			<button class="back_btn" href="<?php echo U('article/index?cate_id='.$cate_id);?>">返 回</button>
			<?php if(C('OPEN_DRAFTBOX') and (ACTION_NAME == 'add' or $info['status'] == 3)): ?><button class="save_btn" url="<?php echo U('article/autoSave');?>" target-form="form-horizontal" id="autoSave">
				存草稿
			</button><?php endif; ?>
			...
			<input type="hidden" name="admin" value="<?php echo ($info["admin"]); ?>">
		</div>
	</form>
	</div>
</div>
