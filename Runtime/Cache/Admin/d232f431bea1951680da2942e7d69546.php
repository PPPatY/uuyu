...
@media only screen and (max-width: 1600px){
    ...
    <div class="fl button_list">
        <?php if(empty($model["extend"])): ?><div class="tools">
                 <a class=" ajax-post " target-form="ids" url="<?php echo U("Promote/set_check_status",array('model'=>'Spend',"status"=>1,"msg_type"=>8,"field"=>"is_check"));?>"><span class="button_icon button_icon10"></span>参与结算</a>
            <a class=" ajax-post " target-form="ids" url="<?php echo U("Promote/set_check_status",array('model'=>'Spend',"status"=>2,"msg_type"=>8,"field"=>"is_check"));?>"><span class="button_icon button_icon11"></span>不参与结算</a>
            </div><?php endif; ?>
        </div>
		<!-- 高级搜索 -->
        ...
        <div class="input-list">
            <label>玩家账号：</label>
            <input type="text"  name="account" class="" value="<?php echo I('account');?>" placeholder="请输入账号"/>
        </div>
        <div class="input-list">
            <label>订单号：</label>
            ...
            <div class="input-list input-list-promote search_label_rehab">
                ...
                <select id="parent_id" name="parent_id" class="select_gallery" >
                    ...
                    <?php $_result=get_all_toppromote();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option promote-id="<?php echo ($vo["id"]); ?>" value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["account"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
             <div class="input-list input-list-promote search_label_rehab">
                <label>推广员账号：</label>
            ...
            <div class="input-list input-list-game search_label_rehab">
                ...
                <select id="id" name="busier_id" class="select_gallery" >
                    ...
                    <?php $_result=get_busier_list();if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option game-id="<?php echo ($vo["id"]); ?>" value="<?php echo ($vo['id']); ?>"><?php echo ($vo['busier_account']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
            <div class="input-list">
                <label>充值IP：</label>
         ...
         <div class="input-list input-list-spend search_label_rehab">
                ...
                <select id="pay_way_id" name="pay_way" class="select_gallery select2-search--hide">
                    ...
                    <?php $_result=all_pay_way(1);if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["key"]); ?>"><?php echo ($vo["value"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
            <input type="hidden" name="" value="" class="sortBy">
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/spend_list','model='.$model['name'],false);?>">搜索</a>
            </div>
            <div class="input-list">
                <a class="export-btn" href="<?php echo U('Export/expUser',array_merge(array('id'=>19,),I('get.')));?>">导出</a><span class="totalvalue">(共计充值<i><?php echo ($total); ?></i>元)</span>
            </div>
		</div>
	</div>
                ...
                <tbody>
                        ...
                        <td>
                            ...
                            <td><?php echo ($data["user_account"]); ?></td>
                             <td><?php echo get_promote_account($data['promote_id']);?></td>

                            <td><?php echo get_promote_parent($data['promote_id'],'account');?></td>
                            ...
                            <td><?php echo get_busier_nickname($data['promote_id']);?></td>
                            <td>
                                <div class="partakebtn">
                               <?php if($data["is_check"] == 1): ?><a class="ajax-post" target-form="ids" href="" url="<?php echo U('Promote/set_check_status',array('model'=>'Spend','ids'=>$data['id'],'status'=>2,'msg_type'=>8,'field'=>'is_check'));?>">参与</a>
                                <?php else: ?>
                                <a  class="ajax-post" target-form="ids" href="" url="<?php echo U('Promote/set_check_status',array('model'=>'Spend','ids'=>$data['id'],'status'=>1,'msg_type'=>8,'field'=>'is_check'));?>" style="color: red">不参与</a><?php endif; ?>
                                </div>
                            </td>
                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <tr class="data_summary">
                        <td colspan="16"><span style="margin-right: 10px;">汇总：</span><span>当页充值：<?php echo null_to_0(array_sum(array_column(array_status2value('pay_status','',$list_data),'pay_amount')));?>元 ； 今日充值：<?php echo ($ttotal); ?>元 ； 昨日充值：<?php echo ($ytotal); ?>元 ； 累计充值：<?php echo ($total); ?>元（此处只汇总充值成功的订单）</span></td>
                    </tr>
									<?php else: ?>
									<tr><td colspan="16">aOh! 暂时还没有内容!</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
    </div>
