...
<script type="text/javascript" src="/Public/Admin/js/select2.min.js"></script>
	...
	<div class="cf top_nav_list">
		...
		<div class="jssearch fr cf search_list">
            <!--  <div class="input-list">
                <label>游戏名称：</label>
                <input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'game_name'); ?>" class="" value="<?php echo I('game_name');?>">
            </div> -->
   ...
   <!--          <div class="input-list">
                <label>礼包名称：</label>
                <input type="text" name="giftbag_name" class="" value="<?php echo I('giftbag_name');?>" placeholder="请输入礼包名">   
            </div> -->
            
            <div class="input-list">
                <label>添加时间：</label>
                <input type="text" id="time-start" name="time-start" class="" value="<?php echo I('time-start');?>" placeholder="起始时间" /> 
                <span style="color: #B3B3B5;font-weight: bold;">—</span>
                <div class="input-append date" id="datetimepicker" style="display:inline">
                <input type="text" id="time-end" name="time-end" class="" value="<?php echo I('time-end');?>" placeholder="结束时间" />
            ...
            <div class="input-list">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Giftbag/lists','model='.$model['name'],false);?>">搜索</a>
            </div>
		</div>
	</div>
    ...
    <div class="data_list">
        <div class="">
            <table>
                ...
                <!-- 列表 -->
                <tbody>
                    <?php if(empty($list_data)): ?><tr><td colspan="10">暂无数据</td></tr>
                    <?php else: ?>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="text-align: center;"><?php echo ($data["giftbag_name"]); ?></td>
                            <td style="text-align: center;"><?php echo $data['is_unicode']==1?'是':'否';?></td>
                            <td style="text-align: center;"><?php if($data['is_unicode'] == 1): echo ($data['unicode_num']); else: echo arr_count($data['novice']); endif; ?></td>
                            <td style="text-align: center;"><?php echo get_info_status($data['giftbag_type'],1);?></td>
                            <td style="text-align: center;"><?php if($data['status'] == 0): ?><span style="color: red"><?php echo get_info_status($data['status']);?></span><?php else: echo get_info_status($data['status']); endif; ?></td>
                            <td style="text-align: center;"><?php echo set_show_time($data['create_time']);?></td>
                            <td style="text-align: center;"><?php echo set_show_time($data['start_time'],'date');?>至 <?php echo set_show_time($data['end_time'],'date','forever');?></td>
                            <td>
                            <a href="<?php echo U('Giftbag/edit?id='.$data['id']);?>">编辑</a>
                            <a href="<?php echo U('Giftbag/del?ids='.$data['id']);?>" class="confirm ajax-get">删除</a>
                            </td>
...
$(function(){
	...
	$("#search").click(function(){
        ...
        var end = ($("#time-end").val() == "") ?  "<?php echo date('Y-m-d');?>" : $("#time-end").val();
