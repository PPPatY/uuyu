...
<!--下拉样式结束-->
    ...
    <div class="tab_nav jstabnav box_mt">
        <ul>
            <?php if(isset($_GET['group'])): ?><li><a href="<?php echo U('index');?>">全部</a></li>
            <?php else: ?>
                <li class="current"><a href="#">全部</a></li><?php endif; ?>
            <?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><li><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a></li>
                <?php else: ?>
                <li class="current"><a href="#"><?php echo ($vo); ?></a></li><?php endif; endforeach; endif; ?>
        </ul>
    </div>
	<div class="cf top_nav_list">
		<div class="fl button_list">
            <a class="" href="<?php echo U('add');?>"><span class="button_icon button_icon1"></span>新 增</a>
            <a class="" href="javascript:;"><span class="button_icon button_icon2"></span>删 除</a>
            <a class=" list_sort" url="<?php echo U('sort?group='.I('group'),'','');?>"><span class="button_icon button_icon18"></span>排序</a>
        </div>
		<!-- 高级搜索 -->
		<div class="jssearch fr cf search_list">
			<div class="sleft">
				...
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('config/index');?>">搜索</a>
			</div>
		</div>
	</div>
		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><tr>
						<td>
							<label class="checked">
								<input class="ids" type="checkbox" name="id[]" value="<?php echo ($config["id"]); ?>">
								<i class="check_icon"></i>
							</label>
						</td>
