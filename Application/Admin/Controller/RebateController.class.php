    ...
    public function lists(){
     ...
     public function add(){
    	if(IS_POST){
           ...
           if($is_set!==null){
                 $this->error("此游戏已经设置过返利");
            }else{
                $add['game_name']=get_game_name($add['game_id']);
								if ($add['status'] == 1) {
									if (!is_numeric($add['money']) || $add['money']<1) {
										$this->error("单笔金额不能为空");
									}									
								}
								if (!is_numeric($add['ratio'])) {
									$this->error("返利比例不能为空");
								}
        ...
        public function edit() {
            ...
            if(IS_POST){
								if ($_POST['status'] == 1) {
									if (!is_numeric($_POST['money']) || $_POST['money']<1) {
										$this->error("单笔金额不能为空");
									}									
								}
								...
								if ($_POST['ratio']<1 || $_POST['ratio']>100) {
									$this->error("返利比例输入错误");
								}
