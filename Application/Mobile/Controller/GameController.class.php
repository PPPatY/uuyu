...
class GameController extends BaseController {
    ...
    private function gameGift($game_id=''){
        ...
        if(ACTION_NAME=='suspension_gift'){
            ...
            $data = $model->getGiftLists($game_id,1,100);
    ...
    private function gameActiveDoc($game_id=''){
        ...
        $data = $model->getArticleListsByCategory($game_id,array('wap_huodong','wap_gg'),1,3);
    ...
    */
    ...
    public function game_pay($developers=''){
        ...
        $paytype = M('tool', 'tab_')->field('status,name')->where(['name'=>['in','weixin,wei_xin,alipay,jft,upay']])->select();
    ...
    */
...

    {
        ...
        $this->assign('game_icon',get_cover($type['icon'],'path'));
        ...
        if(is_weixin()){
            ...
            if(empty(session('shareparams'))){
				...
				if($type['game_name'] == '心动女友') {
					
					$titledata = array('人家好想你呀，快来看看人家吧！','我可以做你的女朋友吗？','我为你而来，你可以来看看我吗');
					$thiskey = array_rand($titledata);
					$title = $titledata[$thiskey];

					$shareparams['title'] =$title;  //$type['game_name']; 

				  
					$shareparams['desc'] = '我正在玩“'.$type['game_name'].'”,快来和我一起玩吧！';  //$desc;

					$shareparams['imgUrl'] = $http.'://'.$_SERVER["HTTP_HOST"].get_cover($type['icon'],'path'); 

					$shareparams['link'] = $full_url;
				} elseif ($type['game_name'] == '花儿消消乐') {
					
					$titledata = array('比开心消消乐好玩一百倍的游戏，等你体验！','这个消消乐玩一次就上瘾！');
					$thiskey = array_rand($titledata);
					$title_1 = $titledata[$thiskey];

					$shareparams['title'] =	$title_1;  //$type['game_name']; 
					...
					$shareparams['link'] = $full_url;
						...
						$titledata = array('天赋异禀的小伙伴，我需要你的支援！','我为你而来，你可以来看看我吗','缺你不行，快来加入我们的战斗');
						$thiskey = array_rand($titledata);
						$title = $titledata[$thiskey];
    ...
    public function create_role(){
        ...
        if(!empty($data)){
            ...
            $userdata = M('User','tab_')
                         ...
                         ->where(array('up.game_id'=>$gamedata['game_id']))
    ...
    public function suspension_gift($game_id){
        ...
        $other = $giftmodel->getGameGiftLists($rec_status,is_login(),array('g.id'=>array('neq',$game_id)));
