    ...
    */
    ...
    public function gameGoldPig(){
     ...
     public function upay($amount=''){
        ...
        if($_REQUEST['code'] == 1){ //充值平台币
            $data['backurl'] = 'http://'.$_SERVER['HTTP_HOST'].'/media.php/Subscriber/user';
            $prefix = 'PF_';
        }else{ //游戏中充值
            $game_data = get_game_entity($_REQUEST["game_appid"]);
            if(empty($game_data)){$this->error("游戏不存在");exit();}
             $data['backurl'] = 'http://'.$_SERVER['HTTP_HOST'].'/media.php/Game/paysdk/code/0/game_appid/'.$_REQUEST["game_appid"];
        ...
        $cardno = 42;
        $out_trade_no = $prefix . date('Ymd') . date('His') . sp_random_string(4);
        $data['customerid'] = C('upay.partner');
        $data['orderAmount'] = $amount*100;
        $data['sdcustomno'] = $out_trade_no;
        $data['cardno'] = $cardno;
        $data['noticeurl'] = 'http://'.$_SERVER['HTTP_HOST'].'/callback.php/Notify/upay_notify';
        $sign_str = "customerid=".$data['customerid']."&sdcustomno=".$data['sdcustomno']."&orderAmount=".$data['orderAmount']."&cardno=".$data['cardno']."&noticeurl=".$data['noticeurl']."&backurl=".$data['backurl'].C('upay.key');
        $data['sign'] = strtoupper(md5($sign_str));
        $data['mark'] = 'xigu2018';
        $data['remarks'] = '游戏充值';
        $data['zftype'] = 1;
        $_payUrl = 'http://api.51upay.com/PayMegerHandler.ashx?'.http_build_query($data);

        //平台支付下单
       
        if($_REQUEST['code'] ==1 ){
            $spend_data['out_trade_no'] = $out_trade_no;
            $spend_data['pay_way'] = 8;
            $spend_data['amount'] = $amount;
            $this->add_deposit($spend_data,$user);
        }else{
            $spend_data['order_number'] = '';
            $spend_data['pay_order_number'] = $out_trade_no;
            $spend_data['user_id'] = $user['id'];
            ...
            $spend_data["game_id"]       = $game_data['id'];
            $spend_data["game_appid"]    = $_REQUEST["game_appid"];
            $spend_data["game_name"]     = $game_data["game_name"];
            $spend_data['props_name']    = $_REQUEST['props_name'];
            $spend_data["server_id"]     = $_REQUEST["server_id"];
            $spend_data["server_name"]   = $_REQUEST["server_name"];
            $spend_data["game_player_id"]     = $_REQUEST["role_id"];
            $spend_data["game_player_name"]   = $_REQUEST["role_name"];
            $spend_data["pay_amount"]   = $amount;
            $spend_data['pay_way'] = 8;
            $spend_data['extend']  = $_REQUEST['trade_no'];
            $this->add_spend($spend_data);
        }
        if(IS_AJAX){
            $this->ajaxReturn(array('status'=>1,'url'=>$_payUrl));
        }else{
            redirect($_payUrl);exit;
        }
     ...
     public function add_spend($data){
        ...
        $spend_data['pay_order_number'] = $data["pay_order_number"];
        $spend_data['props_name']       = $data["props_name"];
