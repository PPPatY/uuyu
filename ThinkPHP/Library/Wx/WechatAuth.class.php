...
class WechatAuth {
	...
	public function getImgUrl($filename){
		...
		$dataurl=array("media"=>'@'. $filename);
		$imgurl=$this->api('media/uploadimg', $dataurl, 'POST', '', false);
		if($imgurl){
			$res=$imgurl['url'];
		}
		return $res;
	...
	//2.新增永久图文素材,返回说明"media_id":MEDIA_ID
	/**
     * 调用微信api获取响应数据
     ...
     */
	public function uploadNews($content){
		
		return $res=$this->newsapi('material/add_news',$content,'post');
		
	}
	

    /**
     * 上传其他类型永久素材（image，thumb,video）
	...
	/*删除永久素材*/
	public function delMeterial($media_id){
	    $data['media_id']=$media_id;
	    return $this->api('material/del_material', $data, 'POST', '', false);
	}
	...
	/* 群发消息给用户(这里单个用户演示) 预览接口正式后只需改动url*/
	public function msgSendAll($content){
		
        $res=$this->api('message/mass/preview',$content,'post');
    ...
    /*图文消息里的内容对html标签处理api*/
    protected function newsapi($name, $data = '', $method = 'POST', $param = '', $json = true){
            $params = array('access_token' => $this->accessToken);
            ...
            if($json && !empty($data)){
                    for($i=0;$i<count($data);$i++){
                        foreach ($data[$i] as $k=>$v){
                            if($k =='content'){
                                $data[$i][$k] = urlencode(htmlspecialchars(str_replace("\"","'",$v)));
                            }else{
                                $data[$i][$k] = urlencode($v);
                            }
                        }
                    }
                    $content=json_encode(array("articles"=>$data));
                    $end_data= htmlspecialchars_decode(urldecode($content));
            }
            
            $data_res = self::http($url, $params, $end_data, $method);
            
            return json_decode($data_res, true);
