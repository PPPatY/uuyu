...
class Wechat {
    ...
    public function response($content, $type = self::MSG_TYPE_TEXT){
       ...
       if($content){ 
            ...
            throw new \Exception('缺少参数Content！');
        }
    ...
    public function replyText($content){
        return $this->response($content, self::MSG_TYPE_TEXT);
    }
     ...
     */
    public function replyNewsOnce($content){
        //return $this->response(array(func_get_args()), self::MSG_TYPE_NEWS);
        return $this->response($content, self::MSG_TYPE_NEWS);
    }
	...
	public function xmlEncode($data=array()){
